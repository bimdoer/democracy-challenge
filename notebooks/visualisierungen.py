...
PLOTLY_BG_TRANSPARENT = "rgba(0,0,0,0)"   # nur für geo.bgcolor / geo.lakecolor
PLOTLY_PAPER_BG = "rgba(255,255,255,0.82)"  # leicht durchscheinend für farbige Seitenhintergründe
...
def gestapeltes_balkendiagramm(df, xlabel="", ylabel="Anteil (%)",
    ...
    if annotate:
        for c in ax.containers:
            labels = [f"{v.get_height():{fmt}}%" if v.get_height() >= min_anteil else "" for v in c]
...
def scatterplot(data, x, y, size=None, titel="", xlabel="", ylabel="",
    ...
    fig, ax = plt.subplots(figsize=figsize)
    _transparent(fig, ax)
    sns.scatterplot(data=data, x=x, y=y, size=size, sizes=sizes,
...
def liniendiagramm(data, x, y, hue=None, titel="", xlabel="", ylabel="",
    ...
    fig, ax = plt.subplots(figsize=figsize)
    _transparent(fig, ax)
    sns.lineplot(data=data, x=x, y=y, hue=hue, palette=palette, color=farbe,
                 marker=marker, linewidth=linewidth,
                 errorbar="ci" if errorbar else None, ax=ax)
...
def liniendiagramm_interaktiv_zeitwahl(
    df, wert_cols, label_map=None, farben_map=None, zeit_spalten=None,
    ...
    if zeit_spalten is None:
        zeit_spalten = {'10 Jahre': 'jahrzehnt', '5 Jahre': '5_jahre', '1 Jahr': 'jahr'}
    ...
    for label, spalte in zeit_spalten.items():
        ...
        for col in wert_cols:
            ...
            fig.add_trace(go.Scatter(
                x=agg[spalte], y=agg[col], mode='lines+markers', name=name,
                line=dict(color=farbe, width=2), marker=dict(size=6),
                opacity=linien_opazitaet, visible=(label == default_label),
    ...
    layout_args = dict(
        title=titel, xaxis_title=xlabel, yaxis_title=ylabel,
        template=PLOTLY_TEMPLATE, font=dict(family=PLOTLY_FONT_FAMILY, size=PLOTLY_FONT_SIZE),
        hovermode="x unified", paper_bgcolor=PLOTLY_PAPER_BG, plot_bgcolor=PLOTLY_PAPER_BG,
        updatemenus=[dict(type="buttons", direction="right", x=0.5, xanchor="center",
                          y=1.15, yanchor="top", buttons=buttons, showactive=True)],
        margin=dict(t=100),
    )
...
def boxplot_facetiert_zeitwahl(
    df, wert_cols, hauptgruppe_spalte, phasen_spalte,
        ...
        for hg_idx, hg in enumerate(hauptgruppen):
            ...
            for akteur_col in wert_cols:
                ...
                fig.add_trace(go.Box(
                    y=df_hg[akteur_col], x=[name] * len(df_hg), name=name,
                    fillcolor=fuellung, line=dict(color=farbe, width=1.2),
                    marker=dict(color=farbe, size=3, opacity=0.75, line=dict(width=0)),
    ...
    buttons = _baue_zeitwahl_buttons(zeit_spalten,
                                     n_traces_pro_label=n_gruppen * n_akteure,
                                     anzahl_dummy_traces=n_akteure)

    if hline is not None:
        for hg_idx in range(n_gruppen):
            fig.add_hline(y=hline, line_dash="dash", line_color=AKZENTFARBE,
                          line_width=1, opacity=0.6,
                          row=hg_idx // n_cols + 1, col=hg_idx % n_cols + 1)

    fig.update_layout(
        title=titel, hovermode=False, template=PLOTLY_TEMPLATE,
        font=dict(family=PLOTLY_FONT_FAMILY, size=PLOTLY_FONT_SIZE_SUBPLOT),
        paper_bgcolor=PLOTLY_PAPER_BG, plot_bgcolor=PLOTLY_PAPER_BG,
        height=hoehe_pro_zeile * n_rows + 120, showlegend=False,
        updatemenus=[dict(type="buttons", direction="right", x=0.5, xanchor="center",
                          y=1.08, yanchor="bottom", buttons=buttons, showactive=True)],
        margin=dict(t=140),
    )
    fig.update_xaxes(categoryorder='array', categoryarray=akteur_namen, title_text="")
    if yrange is not None:
